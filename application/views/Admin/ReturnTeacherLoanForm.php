<script>
    if(1){
    $('#modal-header').html('<b class="box-title">Return Loan</b>');
    }else{
...
<form id="loanreturnform" enctype="multipart/form-data" autocomplete="off">
                <div class="tile-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3">
                            <label>Loan Amount Returned:</label>
                            <input type="number" class="form-control" name="debit" value="<?php echo $total_share[1]==''?0:($total_share[1]*(-1));?>" placeholder="debit">
        ...
        <script>
            ...
            $(document).ready(function(){
             $('form#loanreturnform').submit(function(e){
                    e.preventDefault();
                   if(confirm("Are You sure!....")){
                     $.ajax({
                        type:'POST',
                        url:baseurl+'AdminController/ReturnTeacherLoan',
                        contentType:false,
                        processData:false,
                        cache:true,
        ...
        $(function(){
            ...
            $("th[contenteditable=true]").blur(function(){
                var field_userid = $(this).attr("id") ;
                var value = $(this).text() ;
                $.post(baseurl+'AdminController/ReturnTeacherLoanById' , field_userid + "=" + value, function(data){
                    if(data != '')
        			{
        				message_status.show();
        				message_status.text(data);
